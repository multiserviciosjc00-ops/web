...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['servicio'])) {
    ...
    if ($servicio !== "") {
        ...
        if (!$forzarReinicio) {
            ...
            // Insertar en tabla auxiliar pares/impares
            $tablaDestino = ($nuevoTurnoNum % 2 === 0) ? "turnos_pares" : "turnos_impares";
            $stmt2 = $con->prepare("INSERT INTO $tablaDestino (tipoDocumento, numeroDocumento, servicio, fecha, turno, estado, piso) 
                                    VALUES (?, ?, ?, ?, ?, 'Pendiente', ?)");
            $stmt2->bind_param("ssssis", $tipoDocumento, $numeroDocumento, $servicio, $fecha, $nuevoTurnoNum, $piso);
...
// 🔹 Limpiar solo sesión de Turno (NO cerrar sesión de caja)
if (isset($_GET['clearSession']) && $_GET['clearSession'] == 1) {
    ...
    // Redirige al inicio del turnero pero mantiene caja activa
    header("Location: inicio.php"); 
    exit;
}
...
@media (max-width:600px){
    #btnHome { font-size:1em; padding:10px; }
    #reloj { font-size:0.9em; }
}
</style>
<script>
